...
export default {
	...
	fetchData(options: Object = defaultOptions, type = "nextPage") {
		...
		// swich service
		const service = getServiceByExchange("SEAS", options.Exchange || "");
		...
		return async dispatch => {
			...
			try {
				...
				if (response.status === 200) {
					...
					if( response.data.STATUS !== "ERROR" ) {
						...
						if (response.data.STATUS === "WARNING") {
							...
							messageData = {
								messageTitle: response.data.messageTitle || translate("#$seas$#Cảnh báo"),
								message: response.data.message || translate("#$seas$#Lỗi không xác định"),
								messageType: "WARNING",
								messageHandle: () => {
									dispatch({
										type: `${key}#alert`,
				...
				dispatch({
					...
					payload: {
						messageTitle: "Error",
						message: translate("#$seas$#Lỗi kết nối"),
						messageType: "ERROR",
						messageHandle: () => {
							dispatch({
								type: `${key}#alert`,
