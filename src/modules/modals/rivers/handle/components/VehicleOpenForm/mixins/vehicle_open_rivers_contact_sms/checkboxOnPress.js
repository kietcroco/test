...
export default function onPress() {
	...
	if( !this.state.vehicle_open_rivers_contact_sms.checked ) {

		// nếu đã chọn phone
		if( this.state.vehicle_open_rivers_contact_phone.checked && this.state.vehicle_open_rivers_contact_phone.editable ) {

			editable = false;
		}

		// nếu dữ liệu rỗng thì báo lỗi
		if( !this.state.vehicle_open_rivers_contact_sms.value ) {

			messageType = "error";
			message = `${ translate("Bạn vui lòng nhập số điện thoại") }.`;
		} else if( this.state.vehicle_open_rivers_contact_sms.value.length < 10 ) {
		...
		this.setState({
			...
			if( !editable ) {
				...
				this.refs.vehicle_open_rivers_contact_phone.focus();
				return;
			} 

			// focus sms
			this.refs.vehicle_open_rivers_contact_sms.focus();
		});
	...
	if( 
		...
		// nếu chưa nhập mail
		if( !this.state.vehicle_open_rivers_contact_email.value ) {
	...
	if( !this.state.vehicle_open_rivers_contact_phone.checked && !this.state.vehicle_open_rivers_contact_email.checked ) {
	...
	this.setState({
		...
		if( this.state.vehicle_open_rivers_contact_phone.checked && !this.state.vehicle_open_rivers_contact_phone.value ) {
