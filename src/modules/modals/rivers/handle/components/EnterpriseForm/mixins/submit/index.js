...
export default function submit() {
	...
	/** ## ngành nghề ## */
	let enterprise_rivers_career = null;
	if( this.state.enterprise_rivers_career.value ) {

		enterprise_rivers_career = this.state.enterprise_rivers_career.value.map( item => item.value ).join(";");
	}
	data["enterprise_rivers_career"] = enterprise_rivers_career;

	/** ## Tên công ty ## */
	data["account_company_name_acronym"] = this.state.account_company_name_acronym.value || "";

	/** ## Tên công ty ## */
	data["enterprise_rivers_vehicle_counter"] = this.state.enterprise_rivers_vehicle_counter.value || 0;
