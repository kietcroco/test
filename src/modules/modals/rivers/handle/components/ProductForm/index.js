...
import {
	product_rivers_load_port as mixinsLoadPort,
	product_rivers_discharge_port as mixinsDischargePort,
	product_rivers_type_name as mixinsTypeName,
	product_rivers_weight as mixinsWeight,
	product_rivers_weight_unit as mixinsWeightUnit,
	product_rivers_freight as mixinsFreight,
	product_rivers_loading_time_earliest as mixinsLoadingTime,
	product_rivers_loading_time_latest as mixinsDischargingTime,
	product_rivers_contact_sms as mixinsContactSms,
	product_rivers_contact_phone as mixinsContactPhone,
	product_rivers_contact_email as mixinsContactMail,
	submit
} from './mixins';
			...
			if( !res.didCancel && res.data ) {
				...
				this.setState({
					product_rivers_avatar: {
						...this.state.product_rivers_avatar,
	...
	async _addImage() {
		try {
			...
			if( !res.didCancel && res.data ) {
				...
				const product_rivers_images = this.state.product_rivers_images.slice();

				product_rivers_images.push({
					source: {uri: `data:${ res.type };base64,${ res.data }`},
					info: {
						name: res.fileName,
						size: res.fileSize,
	...
	render() {
		...
		return (
			<View>
				<Avatar
					isHandle
					source 		= { this.state.product_rivers_avatar.source }
					onPress 	= { this._onPressAvatar }
				>{ this.state.product_rivers_avatar.code }</Avatar>

				<TextInput
					ref 			= "product_rivers_load_port"
					label 			= { translate("Nơi nhận hàng") }
					placeholder 	= { translate("Chọn nơi nhận") }
					type 			= "select"
					style 			= { _styles.input }
					returnKeyType 	= "next"
					//messageType 	= { this.state.product_rivers_load_port.messageType }
					value 			= { this.state.product_rivers_load_port.label }
					onPress 		= { mixinsLoadPort.inputOnPress.bind(this) }
					required
				>{ /*this.state.product_rivers_load_port.message*/ }</TextInput>
				<ModalCollapse
					visible 			= { this.state.product_rivers_load_port.modalVisible }
					value 				= { this.state.product_rivers_load_port.value }
					defaultValue 		= { [] }
				...
				<TextInput
					...
					required
				>{ /*this.state.product_rivers_discharge_port.message*/ }</TextInput>
				<ModalCollapse
					visible 			= { this.state.product_rivers_discharge_port.modalVisible }
					value 				= { this.state.product_rivers_discharge_port.value }
					defaultValue 		= { [] }
					source 				= { regionsRiversSource }
				...
				<TextInput
					ref 			= "product_rivers_type_name"
					label 			= { translate("Loại hàng") }
					placeholder 	= { translate("Chọn loại hàng") }
					type 			= "select"
					style 			= { _styles.input }
					returnKeyType 	= "next"
					//messageType 	= { this.state.product_rivers_type_name.messageType }
					value 			= { this.state.product_rivers_type_name.label }
					onPress 		= { mixinsTypeName.inputOnPress.bind(this) }
					required
				>{ /*this.state.product_rivers_type_name.message*/ }</TextInput>
				<ModalCollapse
					visible 			= { this.state.product_rivers_type_name.modalVisible }
					value 				= { this.state.product_rivers_type_name.value }
					defaultValue 		= { [] }
				...
				<View style={ _styles.inputRow }>
					<TextInput
						...
						maxLength 		= { 10 }
						//messageType 	= { this.state.product_rivers_weight.messageType }
						value 			= { this.state.product_rivers_weight.value }
						onChangeText 	= { mixinsWeight.inputOnChangeText.bind(this) }
						onSubmitEditing = { mixinsWeight.inputOnSubmitEditing.bind(this) }
						required
					>{ /*this.state.product_rivers_weight.message*/ }</TextInput>

					<TextInput
						ref 			= "product_rivers_weight_unit"
						label 			= { translate("Chọn ĐVT") }
						placeholder 	= { translate("ĐVT") }
						type 			= "select"
						style 			= { _styles.inputRight }
						returnKeyType 	= "next"
						//messageType 	= { this.state.product_rivers_weight_unit.messageType }
						value 			= { this.state.product_rivers_weight_unit.label }
						onPress 		= { mixinsWeightUnit.inputOnPress.bind(this) }
						required
					>{ /*this.state.product_rivers_weight_unit.message*/ }</TextInput>
					<ModalCollapse
						visible 			= { this.state.product_rivers_weight_unit.modalVisible }
						value 				= { this.state.product_rivers_weight_unit.value }
				...
				<View style={ _styles.inputRow }>
					<TextInput
						...
						keyboardType 	= "numeric"
						maxLength 		= { 19 }
						//messageType 	= { this.state.product_rivers_freight.messageType }
						value 			= { this.state.product_rivers_freight.value }
						onChangeText 	= { mixinsFreight.inputOnChangeText.bind(this) }
						required
					>{ /*this.state.product_rivers_freight.message*/ }</TextInput>
					...
					<AgreementButton 
						checked 	= { this.state.product_rivers_freight.value === 0 || this.state.product_rivers_freight.value === "0" }
						onPress 	= { mixinsFreight.agreeOnPress.bind(this) }
					/>
				</View>

				<TextInput
					ref 			= "product_rivers_loading_time_earliest"
					label 			= { translate("Thời gian xếp hàng sớm nhất") }
					placeholder 	= { translate("Chọn") }
					type 			= "select"
					style 			= { _styles.input }
					returnKeyType 	= "next"
					//messageType 	= { this.state.product_rivers_loading_time_earliest.messageType }
					value 			= { this.state.product_rivers_loading_time_earliest.label }
					onPress 		= { mixinsLoadingTime.inputOnPress.bind(this) }
					required
				>{ /*this.state.product_rivers_loading_time_earliest.message*/ }</TextInput>
				<DatePicker 
					visible 		= { this.state.product_rivers_loading_time_earliest.modalVisible }
					date 			= { this.state.product_rivers_loading_time_earliest.value }
					minDate 		= { now }
					maxDate 		= { this.state.product_rivers_loading_time_latest.value != now ? this.state.product_rivers_loading_time_latest.value : undefined }
				...
				<TextInput
					ref 			= "product_rivers_loading_time_latest"
					label 			= { translate("Thời gian xếp hàng muộn nhất") }
					placeholder 	= { translate("Chọn") }
					type 			= "select"
					style 			= { _styles.input }
					returnKeyType 	= "next"
					//messageType 	= { this.state.product_rivers_loading_time_latest.messageType }
					value 			= { this.state.product_rivers_loading_time_latest.label }
					onPress 		= { mixinsDischargingTime.inputOnPress.bind(this) }
					required
				>{ /*this.state.product_rivers_loading_time_latest.message*/ }</TextInput>
				<DatePicker 
					visible 		= { this.state.product_rivers_loading_time_latest.modalVisible }
					date 			= { this.state.product_rivers_loading_time_latest.value }
					minDate 		= { this.state.product_rivers_loading_time_earliest.value != now ? this.state.product_rivers_loading_time_earliest.value : undefined }
				...
				>
					<View style={ _styles.rowContact }>
						<TouchableOpacity style={ _styles.contact } onPress={ mixinsContactSms.checkboxOnPress.bind(this) }>
							<CheckBox checked={ this.state.product_rivers_contact_sms.checked } />
							<IconSms />
						</TouchableOpacity>

						<TextInput
							//placeholder = { translate("SĐT SMS") }
							ref 			= "product_rivers_contact_sms"
							type 			= "input"
							...
							maxLength 		= { 15 }
							messageType 	= { this.state.product_rivers_contact_sms.messageType }
							value 			= { this.state.product_rivers_contact_sms.value }
							...
							editable 		= { this.state.product_rivers_contact_sms.editable }
							required
						>{ this.state.product_rivers_contact_sms.message }</TextInput>
					</View>
					<View style={ _styles.rowContact }>
						<TouchableOpacity style={ _styles.contact } onPress={ mixinsContactPhone.checkboxOnPress.bind(this) }>
							<CheckBox checked={ this.state.product_rivers_contact_phone.checked } />
							<IconPhone style={ _styles.iconContact }/>
						</TouchableOpacity>

						<TextInput
							//placeholder = { translate("SĐT") }
							ref 			= "product_rivers_contact_phone"
							type 			= "input"
							style 			= { _styles.inputContact }
							inputStyle 		= { _styles.inputContactStyle }
							returnKeyType 	= "next"
							keyboardType 	= "phone-pad"
							maxLength 		= { 15 }
							messageType 	= { this.state.product_rivers_contact_phone.messageType }
							value 			= { this.state.product_rivers_contact_phone.value }
							editable 		= { this.state.product_rivers_contact_phone.editable }
							onBlur 			= { mixinsContactPhone.inputOnBlur.bind(this) }
							onChangeText 	= { mixinsContactSms.inputOnChangeText.bind(this) }
							onSubmitEditing = { mixinsContactPhone.inputOnSubmitEditing.bind(this) }
							required
						>{ this.state.product_rivers_contact_phone.message }</TextInput>
					</View>
					<View style={ _styles.rowContact }>
						<View style={ _styles.contact }>
							<CheckBox disable checked={ this.state.product_rivers_contact_email.checked } />
							<IconEmail style={ _styles.iconContact }/>
						</View>

						<TextInput
							//placeholder = { translate("Email") }
							ref 			= "product_rivers_contact_email"
							type 			= "input"
							style 			= { _styles.inputContact }
							inputStyle 		= { _styles.inputContactStyle }
							returnKeyType 	= "next"
							...
							maxLength 		= { 254 }
							messageType 	= { this.state.product_rivers_contact_email.messageType }
							value 			= { this.state.product_rivers_contact_email.value }
							...
							required
						>{ this.state.product_rivers_contact_email.message }</TextInput>
					</View>
				...
				>
					<TextInput
						ref 			= "product_rivers_information"
						...
						maxLength 		= {1000}
						returnKeyType 	= "done"
						value 			= { this.state.product_rivers_information }
						onChangeText 	= { text => this.setState({
							product_rivers_information: text
						}) }
					></TextInput>
					{/*
					...
					*/}
					<View style={ _styles.imageRow }>
						{
							this.state.product_rivers_images.map( ( {source = null, info = {}}, index ) => {

								const onRemove = () => {

									const product_rivers_images = this.state.product_rivers_images.slice();
									product_rivers_images.splice(index, 1);
								...
								return (
									<AddImage
										key 		= {`product_rivers_images-${ index }`}
										source 		= { source }
										onError 	= { onRemove }
										onRemove 	= { onRemove }
										onPress 	= { async () => {
