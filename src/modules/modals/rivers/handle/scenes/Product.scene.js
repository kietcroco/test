...
class Product extends React.Component {
	...
	static displayName = '@ProductHandle';
				...
				if (res.data.STATUS === "OK" || res.data.STATUS === "NONAUTHORITATIVE" && res.data.data) {
					...
					if (res.data.data.account) {
						...
						if (res.data.data.account.status) {
							...
							// qua trang chi tiết
							return this.props.navigation.replace("/rivers/detail/product", {
								source: res.data.data,
								id: res.data.data.id || res.data.data.product_rivers_id
							});
						}
	...
	async _loadSource() {
		...
		try {
			...
			const res = await productService.handle(id);
	...
	render() {
		...
		return (
			...
			>
				<Banner exchange="rivers">{translate("Hàng - offer")}</Banner>
