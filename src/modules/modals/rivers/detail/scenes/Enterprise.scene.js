...
class Enterprise extends React.Component {
	...
	static navigationOptions = ({ navigation }) => {
		...
		const {
			urlDetail,
			enterprise_rivers_title: title,
			exchanges,
			enterprise_rivers_is_deleted: isDelete,
			enterprise_rivers_created_by: createdBy,
			enterprise_rivers_id: id,
			account: {
			...
			enterprise_rivers_description: description,
			enterprise_rivers_creation_time: creationTime
		} = source;
			...
			headerRight: (
				<View style={_styles.headerRight}>
					...
					{!!isMenuAction && <MenuAction onEdit={() => {
						navigation.navigate('/rivers/handle/enterprise', { id: id })
					}} />}
				</View>
			),
			footerContent: (
				<View style={_styles.footer} pointerEvents="box-none">
					<FooterContact
						...
						exchange 		= { exchanges }
					/>
					<ActionButton onPress={() => navigation.navigate('/rivers/handle/enterprise')} style={_styles.actionButton} />
				</View>
			)
		});
	};
		...
		return relations.map((post, index) => {
			...
			return (
				<RowItem key={`RelationsItem-Rivers-${index}`} source={post} onPress={() => {
					setTimeout(() => this.props.navigation.replace("/rivers/detail/enterprise", {
						source: post,
						id: post.id || post.enterprise_rivers_id
					}), 10);
				}} />
			);
		});
	...
	render() {
		...
		const images = getImagesFromSource(source, source.enterprise_rivers_title);
		//const tags = (source.enterprise_rivers_tag || "").split(",");
		...
		const extraPoster = [{
			...
			value: source.enterprise_rivers_career
		}];
		...
		return (
			...
			>
				...
				<View style={_styles.noteWrapper}>
					...
					<Text style={_styles.text}>{source.account && source.account.account_company_introduce || source.enterprise_rivers_description}</Text>

					<View style={ _styles.viewedContainer }>
						<FAIcon name="eye" style={ _styles.viewedIcon }> {source.enterprise_rivers_viewed || 1}</FAIcon>
					</View>
				</View>
