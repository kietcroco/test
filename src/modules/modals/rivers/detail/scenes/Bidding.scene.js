...
class Bidding extends React.Component {
	...
	static navigationOptions = ({ navigation }) => {
		...
		const {
			source: {
				urlDetail,
				bidding_rivers_title_own: title,
				bidding_rivers_contact_phone: phoneNumber,
				bidding_rivers_contact_email: email,
				bidding_rivers_contact_by: contactBy,
				bidding_rivers_is_deleted: isDelete,
				bidding_rivers_created_by: createdBy,
				bidding_rivers_id: id,
				bidding_rivers_code: code,
				bidding_rivers_description: description,
				bidding_rivers_creation_time: creationTime,
				exchanges,
				bidding_rivers_skype: skype
			} = {},
			...
			headerRight: (
				<View style={_styles.headerRight}>
					...
					{!!isMenuAction && <MenuAction onDelete={() => {
						...
						navigation.navigate('/rivers/handle/bidding', { id: id })
					}} />}
				</View>
			),
			footerContent: (
				<View style={_styles.footer} pointerEvents="box-none">
					<FooterContact
						...
						exchange 		= { exchanges }
					/>
					<ActionButton onPress={() => navigation.navigate('/rivers/handle/bidding')} style={_styles.actionButton} />
				</View>
			)
		});
	};
	...
	componentWillReceiveProps(nextProps) {
		...
		if (this.props.token !== nextProps.token) {
			...
			fetchData(biddingService, nextProps.navigation);
		}
		...
		return relations.map((post, index) => {
			...
			return (
				<RowItem key={`RelationsItem-Rivers-${index}`} source={post} onPress={() => {
					//this.props.navigation.replace('/rivers/detail/bidding', { id: post.id });
					setTimeout(() => this.props.navigation.replace("/rivers/detail/bidding", {
						source: post,
						id: post.id || post.bidding_rivers_id
					}), 10);
				}}
				/>
			);
	...
	render() {
		...
		const images = getImagesFromSource(source, source.bidding_rivers_title_own);
		const tags = (source.bidding_rivers_tag || "").split(",");
		...
		return (
			<ScrollView
				...
				refreshControl={
					<RefreshControl
						...
						onRefresh={() => fetchData(biddingService, navigation, true)}
			...
			>
				...
				<View style={_styles.noteWrapper}>
					...
					<Text style={_styles.text}>{source.bidding_rivers_information || source.bidding_rivers_description}</Text>
					
					<View style={ _styles.viewedContainer }>
						<FAIcon name="eye" style={ _styles.viewedIcon }> {source.bidding_rivers_viewed || 1}</FAIcon>
					</View>
				</View>
				{/*
	...
	componentDidMount() {
		...
		InteractionManager.runAfterInteractions(() => fetchData(biddingService, this.props.navigation));
	}
