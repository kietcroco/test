...
import purchaseService from '~/services/rivers/purchase';
...
class Purchase extends React.Component {
	...
	static navigationOptions = ({ navigation }) => {
		...
		const {
			source: {
				urlDetail,
				purchase_rivers_title: title,
				purchase_rivers_contact_phone: phoneNumber,
				purchase_rivers_contact_email: email,
				purchase_rivers_contact_by: contactBy,
				purchase_rivers_is_deleted: isDelete,
				purchase_rivers_created_by: createdBy,
				purchase_rivers_id: id,
				purchase_rivers_code: code,
				purchase_rivers_description: description,
				purchase_rivers_creation_time: creationTime,
				exchanges,
				purchase_rivers_skype: skype
			} = {},
			...
			headerRight: (
				<View style={_styles.headerRight}>
					...
					{!!isMenuAction && <MenuAction onDelete={() => {
						...
						navigation.navigate('/rivers/handle/purchase', { id: id })
					}} />}
				</View>
			),
			footerContent: (
				<View style={_styles.footer} pointerEvents="box-none">
					<FooterContact
						...
						exchange 		= { exchanges }
					/>
					<ActionButton onPress={() => navigation.navigate('/rivers/handle/purchase')} style={_styles.actionButton} />
				</View>
			)
		});
	};
		...
		return relations.map((post, index) => {
			...
			return (
				<RowItem key={`RelationsItem-Rivers-${index}`} source={post} onPress={() => {
					setTimeout(() => this.props.navigation.replace("/rivers/detail/purchase", {
						source: post,
						id: post.id || post.purchase_rivers_id
					}), 10);
				}} />
			);
		});
	...
	render() {
		...
		const images = getImagesFromSource(source, source.purchase_rivers_title);
		const tags = (source.purchase_rivers_tag || "").split(",");
		...
		return (
			...
			>
				...
				<View style={_styles.noteWrapper}>
					...
					{!!source.purchase_rivers_size_tunnel && 
						<Text style={_styles.text}>{`- ${translate("Kích thước")}: ${source.purchase_rivers_size_tunnel}`}</Text>}
					<Text style={_styles.text}>{source.purchase_rivers_information || source.purchase_rivers_description}</Text>

					<View style={ _styles.viewedContainer }>
						<FAIcon name="eye" style={ _styles.viewedIcon }> {source.purchase_rivers_viewed || 1}</FAIcon>
					</View>
				</View>
				{/*
