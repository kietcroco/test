...
export default function onPress() {
	...
	let message = "";
	let contact_message = "";
	let isEmailDetect = isEmail( this.state.container_seas_contact_email.value );
	let messageTypeMail = null;
	let messageMail = "";

	// nếu chưa check sms (checked)
	if( !this.state.container_seas_contact_sms.checked ) {

		// nếu đã chọn phone
		if( this.state.container_seas_contact_phone.checked && this.state.container_seas_contact_phone.editable ) {

			editable = false;
		}

		// nếu dữ liệu rỗng thì báo lỗi
		if( !this.state.container_seas_contact_sms.value ) {

			messageType = "error";
			message = `${ translate("#$seas$#Bạn vui lòng nhập số điện thoại") }.`;
		} else if( this.state.container_seas_contact_sms.value.length < 10 ) {
		...
		this.setState({
			...
			if( !editable ) {
				...
				this.refs.container_seas_contact_phone.focus();
				return;
			} 

			// focus sms
			this.refs.container_seas_contact_sms.focus();
		});
	...
	if( 
		!this.state.container_seas_contact_phone.checked &&
		!this.state.container_seas_contact_sms.checked
	) {

		// nếu chưa nhập mail
		if( !this.state.container_seas_contact_email.value ) {
	...
	if( !this.state.container_seas_contact_phone.checked && !this.state.container_seas_contact_email.checked ) {

		// báo lỗi bắt buộc chọn 1
		contact_message = `${ translate("#$seas$#Bạn không muốn giao dịch bằng SMS, Điện thoại mời bạn nhập Email") }.`;
	}

	if( this.state.container_seas_contact_phone.checked ) {

		editable = false;

		// nếu dữ liệu rỗng thì báo lỗi
		if( !this.state.container_seas_contact_phone.value ) {

			messageType = "error";
			message = `${ translate("#$seas$#Bạn vui lòng nhập số điện thoại") }.`;
		} else if( this.state.container_seas_contact_phone.value.length < 10 ) {
	...
	this.setState({
		...
		if( this.state.container_seas_contact_phone.checked && !this.state.container_seas_contact_phone.value ) {

			this.refs.container_seas_contact_phone.focus();
		} else if(messageType || contact_message) { // focus vào email nếu có lỗi

			this.refs.container_seas_contact_email.focus();
		}
	}); 
};