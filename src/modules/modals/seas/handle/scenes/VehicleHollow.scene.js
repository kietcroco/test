...
class VehicleHollow extends React.Component {
				...
				if (res.data.STATUS === "OK" || res.data.STATUS === "NONAUTHORITATIVE" && res.data.data) {
					...
					if ( res.data.data.account ) {
						...
						if( res.data.data.account.status ) {
							...
							// qua trang chi tiết
							return this.props.navigation.replace("/seas/detail/vehicle-hollow", {
								source: res.data.data,
								id: res.data.data.id || res.data.data.vehicle_hollow_seas_id
							});
						}
	...
	render() {
		...
		return (
			...
			>
				<Banner exchange="seas">{translate("#$seas$#Tàu chạy rỗng")}</Banner>
