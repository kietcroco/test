...
class VehicleOpen extends React.Component {
	...
	static navigationOptions = ({ navigation }) => {
		...
		const {
			source: {
				urlDetail,
				vehicle_open_seas_title: title,
				exchanges,
				vehicle_open_seas_contact_phone: phoneNumber,
				vehicle_open_seas_contact_email: email,
				vehicle_open_seas_skype: skype,
				vehicle_open_seas_contact_by: contactBy,
				vehicle_open_seas_is_deleted: isDelete,
				vehicle_open_seas_created_by: createdBy,
				vehicle_open_seas_id: id,
				vehicle_open_seas_code: code,
				vehicle_open_seas_description: description,
				vehicle_open_seas_creation_time: creationTime
			} = {},
		...
		return ({
			...
			headerRight: (
				<View style={_styles.headerRight}>
					...
					{!!isMenuAction && <MenuAction 
						...
						onEdit={() => {
							navigation.navigate('/seas/handle/vehicle-open', { id: id })
						}} 
			...
			footerContent: (
				<View style={_styles.footer} pointerEvents="box-none">
					...
					/>
					<ActionButton 
						onPress = {() => navigation.navigate('/seas/handle/vehicle-open')} 
		...
		return relations.map((post, index) => {
			...
			return (
				<RowItem key={`RelationsItem-Seas-${index}`} source={post} onPress={() => {
					setTimeout(() => this.props.navigation.replace("/seas/detail/vehicle-open", {
						source: post,
						id: post.id || post.vehicle_open_seas_id
					}), 10);
				}} />
			);
		});
		...
		return (
			...
			>
				...
				<View style={_styles.noteWrapper}>
					...
					<Text style={_styles.text}>{source.vehicle_open_seas_information || source.vehicle_open_seas_description}</Text>

					<View style={ _styles.viewedContainer }>
						<FAIcon name="eye" style={ _styles.viewedIcon }> {source.vehicle_open_seas_viewed || 1}</FAIcon>
					</View>
				</View>
				{/*
