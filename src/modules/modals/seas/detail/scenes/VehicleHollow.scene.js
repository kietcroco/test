...
import vehicleHollowService from '~/services/seas/vehicleHollow';
...
class VehicleHollow extends React.Component {
	...
	static navigationOptions = ({ navigation }) => {
		...
		const {
			source: {
				urlDetail,
				vehicle_hollow_seas_title: title,
				exchanges,
				vehicle_hollow_seas_contact_phone: phoneNumber,
				vehicle_hollow_seas_contact_email: email,
				vehicle_hollow_seas_skype: skype,
				vehicle_hollow_seas_contact_by: contactBy,
				vehicle_hollow_seas_is_deleted: isDelete,
				vehicle_hollow_seas_created_by: createdBy,
				vehicle_hollow_seas_id: id,
				vehicle_hollow_seas_code: code,
				vehicle_hollow_seas_description: description,
				vehicle_hollow_seas_creation_time: creationTime
			} = {},
		...
		return ({
			...
			headerRight: (
				<View style={_styles.headerRight}>
					...
					{!!isMenuAction && <MenuAction 
						...
						onEdit={() => {
							navigation.navigate('/seas/handle/vehicle-hollow', { id: id })
						}} 
			...
			footerContent: (
				<View style={_styles.footer} pointerEvents="box-none">
					...
					/>
					<ActionButton 
						onPress = {() => navigation.navigate('/seas/handle/vehicle-hollow')}
		...
		return relations.map((post, index) => {
			...
			return (
				<RowItem key={`RelationsItem-Seas-${index}`} source={post} onPress={() => {
					setTimeout(() => this.props.navigation.replace("/seas/detail/vehicle-hollow", {
						source: post,
						id: post.id || post.vehicle_hollow_seas_id
					}), 10);
				}} />
			);
		});
	...
	render() {
		...
		const images = getImagesFromSource(source, source.vehicle_hollow_seas_title);
		const tags = (source.vehicle_hollow_seas_tag || "").split(",");
		...
		return (
			...
			>
				...
				<View style={_styles.noteWrapper}>
					...
					<Text style={_styles.text}>{source.vehicle_hollow_seas_information || source.vehicle_hollow_seas_description}</Text>

					<View style={ _styles.viewedContainer }>
						<FAIcon name="eye" style={ _styles.viewedIcon }> {source.vehicle_hollow_seas_viewed || 1}</FAIcon>
					</View>
				</View>
				{/*
