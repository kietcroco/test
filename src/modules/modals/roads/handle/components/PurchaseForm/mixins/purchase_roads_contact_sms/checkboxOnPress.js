...
export default function onPress() {
	...
	if( !this.state.purchase_roads_contact_sms.checked ) {
		...
		// nếu đã chọn phone
		if( this.state.purchase_roads_contact_phone.checked && this.state.purchase_roads_contact_phone.editable ) {
		...
		this.setState({
			...
			if( !editable ) {
				...
				this.refs.purchase_roads_contact_phone.focus();
				return;
			} 

			// focus sms
			this.refs.purchase_roads_contact_sms.focus();
		});
	...
	if( 
		...
		// nếu chưa nhập mail
		if( !this.state.purchase_roads_contact_email.value ) {
	...
	if( !this.state.purchase_roads_contact_phone.checked && !this.state.purchase_roads_contact_email.checked ) {

		// báo lỗi bắt buộc chọn 1
		contact_message = `${ translate("Bạn không muốn giao dịch bằng SMS, Điện thoại mời bạn nhập Email") }.`;
	}

	if( this.state.purchase_roads_contact_phone.checked ) {

		editable = false;

		// nếu dữ liệu rỗng thì báo lỗi
		if( !this.state.purchase_roads_contact_phone.value ) {

			messageType = "error";
			message = `${ translate("Bạn vui lòng nhập số điện thoại") }.`;
		} else if( this.state.purchase_roads_contact_phone.value.length < 10 ) {
	...
	this.setState({
		...
		if( this.state.purchase_roads_contact_phone.checked && !this.state.purchase_roads_contact_phone.value ) {
