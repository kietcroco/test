...
class VehicleHollow extends React.Component {
				...
				if (res.data.STATUS === "OK" || res.data.STATUS === "NONAUTHORITATIVE" && res.data.data) {
					...
					if (res.data.data.account) {
						...
						if (res.data.data.account.status) {
							...
							// qua trang chi tiết
							return this.props.navigation.replace("/roads/detail/vehicle-hollow", {
								source: res.data.data,
								id: res.data.data.id || res.data.data.vehicle_hollow_roads_id
							});
						}
	...
	render() {
		...
		return (
			...
			>
				<Banner exchange="roads">{translate("Xe chạy rỗng cần hàng")}</Banner>
