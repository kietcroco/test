...
class Purchase extends React.Component {
	...
	static navigationOptions = ({ navigation }) => {
		...
		const {
			source: {
				urlDetail,
				purchase_roads_title: title,
				purchase_roads_contact_phone: phoneNumber,
				purchase_roads_contact_email: email,
				purchase_roads_contact_by: contactBy,
				purchase_roads_is_deleted: isDelete,
				purchase_roads_created_by: createdBy,
				purchase_roads_id: id,
				purchase_roads_code: code,
				purchase_roads_description: description,
				purchase_roads_creation_time: creationTime,
				exchanges,
				purchase_roads_skype: skype
			} = {},
			...
			headerRight: (
				<View style={_styles.headerRight}>
					...
					{!!isMenuAction && <MenuAction onDelete={() => {
						...
						navigation.navigate('/roads/handle/purchase', { id: id })
					}} />}
				</View>
			),
			footerContent: (
				<View style={_styles.footer} pointerEvents="box-none">
					<FooterContact
						...
						exchange 		= { exchanges }
					/>
					<ActionButton onPress={() => navigation.navigate('/roads/handle/purchase')} style={_styles.actionButton} />
				</View>
			)
		});
	};
		...
		return relations.map((post, index) => {
			...
			return (
				<RowItem key={`RelationsItem-Roads-${index}`} source={post} onPress={() => {
					setTimeout(() => this.props.navigation.replace("/roads/detail/purchase", {
						source: post,
						id: post.id || post.purchase_roads_id
					}), 10);
				}} />
			);
		});
	...
	render() {
		...
		const images = getImagesFromSource(source, source.purchase_roads_title);
		const tags = (source.purchase_roads_tag || "").split(",");
		...
		return (
			...
			>
				...
				<View style={_styles.noteWrapper}>
					...
					<Text style={_styles.text}>{source.purchase_roads_information || source.purchase_roads_description}</Text>

					<View style={ _styles.viewedContainer }>
						<FAIcon name="eye" style={ _styles.viewedIcon }> {source.purchase_roads_viewed || 1}</FAIcon>
					</View>
				</View>
				{/*
