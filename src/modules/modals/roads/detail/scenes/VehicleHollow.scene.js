...
import vehicleHollowService from '~/services/roads/vehicle-hollow';
...
class VehicleHollow extends React.Component {
	...
	static navigationOptions = ({ navigation }) => {
		...
		const {
			source: {
				urlDetail,
				vehicle_hollow_roads_title: title,
				exchanges,
				vehicle_hollow_roads_contact_phone: phoneNumber,
				vehicle_hollow_roads_contact_email: email,
				vehicle_hollow_roads_skype: skype,
				vehicle_hollow_roads_contact_by: contactBy,
				vehicle_hollow_roads_is_deleted: isDelete,
				vehicle_hollow_roads_created_by: createdBy,
				vehicle_hollow_roads_id: id,
				vehicle_hollow_roads_code: code,
				vehicle_hollow_roads_description: description,
				vehicle_hollow_roads_creation_time: creationTime
			} = {},
			...
			headerRight: (
				<View style={_styles.headerRight}>
					...
					{!!isMenuAction && <MenuAction onDelete={() => {
						...
						navigation.navigate('/roads/handle/vehicle-hollow', { id: id })
					}} />}
				</View>
			),
			footerContent: (
				<View style={_styles.footer} pointerEvents="box-none">
					<FooterContact
						...
						exchange 		= { exchanges }
					/>
					<ActionButton onPress={() => navigation.navigate('/roads/handle/vehicle-hollow')} style={_styles.actionButton} />
				</View>
			)
		});
	};
		...
		return relations.map((post, index) => {
			...
			return (
				<RowItem key={`RelationsItem-Roads-${index}`} source={post} onPress={() => {
					setTimeout(() => this.props.navigation.replace("/roads/detail/vehicle-hollow", {
						source: post,
						id: post.id || post.vehicle_hollow_roads_id
					}), 10);
				}} />
			);
		});
		...
		return (
			...
			>
				...
				<View style={_styles.noteWrapper}>
					...
					<Text style={_styles.text}>{source.vehicle_hollow_roads_information || source.vehicle_hollow_roads_description}</Text>

					<View style={ _styles.viewedContainer }>
						<FAIcon name="eye" style={ _styles.viewedIcon }> {source.vehicle_hollow_roads_viewed || 1}</FAIcon>
					</View>
				</View>
				{/*
