...
import vehicleOpenService from '~/services/roads/vehicle-open';
...
class VehicleOpen extends React.Component {
	...
	static navigationOptions = ({ navigation }) => {
		...
		const {
			source: {
				urlDetail,
				vehicle_open_roads_title: title,
				exchanges,
				vehicle_open_roads_contact_phone: phoneNumber,
				vehicle_open_roads_contact_email: email,
				vehicle_open_roads_skype: skype,
				vehicle_open_roads_contact_by: contactBy,
				vehicle_open_roads_is_deleted: isDelete,
				vehicle_open_roads_created_by: createdBy,
				vehicle_open_roads_id: id,
				vehicle_open_roads_code: code,
				vehicle_open_roads_description: description,
				vehicle_open_roads_creation_time: creationTime
			} = {},
			...
			headerRight: (
				<View style={_styles.headerRight}>
					...
					{!!isMenuAction && <MenuAction onDelete={() => {
						...
						navigation.navigate('/roads/handle/vehicle-open', { id: id })
					}} />}
				</View>
			),
			footerContent: (
				<View style={_styles.footer} pointerEvents="box-none">
					<FooterContact
						...
						exchange 		= { exchanges }
					/>
					<ActionButton onPress={() => navigation.navigate('/roads/handle/vehicle-open')} style={_styles.actionButton} />
				</View>
			)
		});
	};
		...
		return relations.map((post, index) => {
			...
			return (
				<RowItem key={`RelationsItem-Roads-${index}`} source={post} onPress={() => {
					setTimeout(() => this.props.navigation.replace("/roads/detail/vehicle-open", {
						source: post,
						id: post.id || post.vehicle_open_roads_id
					}), 10);
				}} />
			);
		});
	...
	render() {
		...
		const images = getImagesFromSource(source, source.vehicle_open_roads_title);
		const tags = (source.vehicle_open_roads_tag || "").split(",");
		...
		return (
			...
			>
				...
				<View style={_styles.noteWrapper}>
					...
					<Text style={_styles.text}>{source.vehicle_open_roads_information || source.vehicle_open_roads_description}</Text>

					<View style={ _styles.viewedContainer }>
						<FAIcon name="eye" style={ _styles.viewedIcon }> {source.vehicle_open_roads_viewed || 1}</FAIcon>
					</View>
				</View>
				{/*
