...
import containerService from '~/services/roads/container';
...
class Container extends React.Component {
	...
	static navigationOptions = ({ navigation }) => {
		...
		const {
			source: {
				urlDetail,
				container_roads_title: title,
				exchanges,
				container_roads_contact_phone: phoneNumber,
				container_roads_contact_email: email,
				container_roads_skype: skype,
				container_roads_contact_by: contactBy,
				container_roads_is_deleted: isDelete,
				container_roads_created_by: createdBy,
				container_roads_id: id,
				container_roads_code: code,
				container_roads_description: description,
				container_roads_creation_time: creationTime
			} = {},
			...
			headerRight: (
				<View style={_styles.headerRight}>
					...
					{!!isMenuAction && <MenuAction onDelete={() => {
						...
						navigation.navigate('/roads/handle/container', { id: id })
					}} />}
				</View>
			),
			footerContent: (
				<View style={_styles.footer} pointerEvents="box-none">
					...
					/>
					<ActionButton
						onPress={() => navigation.navigate('/roads/handle/container')}
		...
		return relations.map((post, index) => {
			...
			return (
				<RowItem key={`RelationsItem-Roads-${index}`} source={post} onPress={() => {
					setTimeout(() => this.props.navigation.replace("/roads/detail/container", {
						source: post,
						id: post.id || post.container_roads_id
					}), 10);
				}} />
			);
		});
	...
	render() {
		...
		const images = getImagesFromSource(source, source.container_roads_title);
		const tags = (source.container_roads_tag || "").split(",");
		...
		return (
			...
			>
				...
				<View style={_styles.noteWrapper}>
					...
					<Text style={_styles.text}>{source.container_roads_information || source.container_roads_description}</Text>

					<View style={ _styles.viewedContainer }>
						<FAIcon name="eye" style={ _styles.viewedIcon }> {source.container_roads_viewed || 1}</FAIcon>
					</View>
				</View>
				{/*
