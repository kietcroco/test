...
class Product extends React.Component {
	...
	static navigationOptions = ({ navigation }) => {
		...
		const {
			source: {
				urlDetail,
				product_roads_title: title,
				exchanges,
				product_roads_contact_phone: phoneNumber,
				product_roads_contact_email: email,
				product_roads_skype: skype,
				product_roads_contact_by: contactBy,
				product_roads_is_deleted: isDelete,
				product_roads_created_by: createdBy,
				product_roads_id: id,
				product_roads_code: code,
				product_roads_description: description,
				product_roads_creation_time: creationTime
			} = {},
			...
			headerRight: (
				<View style={_styles.headerRight}>
					...
					{!!isMenuAction && <MenuAction onDelete={() => {
						...
						navigation.navigate('/roads/handle/product', { id: id })
					}} />}
				</View>
			),
			footerContent: (
				<View style={_styles.footer} pointerEvents="box-none">
					...
					/>
					<ActionButton
						onPress={() => navigation.navigate('/roads/handle/product')}
		...
		return relations.map((post, index) => {
			...
			return (
				<RowItem key={`RelationsItem-Roads-${index}`} source={post} onPress={() => {
					setTimeout(() => this.props.navigation.replace("/roads/detail/product", {
						source: post,
						id: post.id || post.product_roads_id
					}), 10);
				}} />
			);
		});
	...
	render() {
		...
		const images = getImagesFromSource(source, source.product_roads_title);
		const tags = (source.product_roads_tag || "").split(",");
		...
		return (
			...
			>
				...
				<View style={_styles.noteWrapper}>
					...
					<Text style={_styles.text}>{source.product_roads_information || source.product_roads_description}</Text>

					<View style={ _styles.viewedContainer }>
						<FAIcon name="eye" style={ _styles.viewedIcon }> {source.product_roads_viewed || 1}</FAIcon>
					</View>
				</View>
				{/*
