...
const Enterprise = ( props ) => {
	...
	let {
        //enterprise_roads_creation_time = 0,
        account: {
        ...
        enterprise_roads_vehicle_counter = 0
    } = source;
    
	//enterprise_roads_creation_time = formatTimeAgo( enterprise_roads_creation_time );
    enterprise_roads_vehicle_counter = formatNumber( enterprise_roads_vehicle_counter, locale );
        ...
        <View style={[ _styles.wrapper, style ]}>
                ...
                
            {
				(!!enterprise_roads_vehicle_counter) &&
                    <Text style={ _styles.textRow }>
                        <FAIcon name='truck' style={_styles.icon} />
                        { _styles.TABSYMBOL }
                        <Text style={ _styles.text }>
                            { translate("Số lượng") }:
                            { _styles.SPACESYMBOL }
			...
			{/*
				!!enterprise_roads_creation_time && 
					<Text style={ _styles.txtTimeAgo }>{ enterprise_roads_creation_time }</Text>
			*/}
		</View>
	);
};
