...
class ModuleTransitioner extends React.PureComponent {

	static displayName = "@ModuleTransitioner";
	...
	_getModuleName( routeName: String = "" ) {

		const module = routeName.split("/");
		module.shift();
	...
	*/
	_renderScene( transitionProps, scene ) {
		...
		const moduleName = this._getModuleName( scene.route.routeName );
		let translateY = 0;

		if( this._lastModuleVisit !== moduleName ) {

			this._lastModuleVisit = moduleName;
		...
		return (
			<Animated.View style={[ _styles.container, {transform: [{ translateY }]}]} key={scene.key}>
...
export default ModuleTransitioner;